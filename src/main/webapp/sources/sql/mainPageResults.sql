...
from 
    (select mygames.*, e.name, ge.ge_id, ge.gameEventId, ge.periodNumber, score.homePeriodScore homePeriodScore, score.awayPeriodScore awayPeriodScore, 
        ...
        from
            (select g.g_id, g.gameType, g.season, g.gameDate, awayTeam.name as awayTeamName, g.awayScore, homeTeam.name as homeTeamName, g.homeScore, v.name as venueName, s.name as statusName
            ...
            left join (select periodScore.g_id, periodScore.periodNumber, count(case periodScore.scoringTeam when periodScore.homeTeam then 1 end) as homePeriodScore, 
...
order by myevents.gameDate desc, myevents.g_id desc, myevents.gameEventId, ep.role